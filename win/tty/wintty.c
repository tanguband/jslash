...
static void FDECL(process_text_window, (winid,struct WinDesc *));static tty_menu_item *FDECL(reverse, (tty_menu_item *));
...
char** argv;
{
    ...
    tty_putstr(BASE_WINDOW, 0, "This is SuperLotsoAddedStuffHack, a modified version of NetHack 3.2.1.");
    tty_putstr(BASE_WINDOW, 0, "Saved games, level files, dungeon files, and bones are incompatible with");
    tty_putstr(BASE_WINDOW, 0, "any other version of Nethack or SLASH. See slashfaq.txt for more info.");
    tty_putstr(BASE_WINDOW, 0, "Slash is a modified Nethack 3.1.3 version by Tom Proudfoot");
    tty_putstr(BASE_WINDOW, 0, "");
#if 0
    tty_putstr(BASE_WINDOW, 0, "Tiles were drawn by Nathan La(nudge@ucla.edu)");
    tty_putstr(BASE_WINDOW, 0, "Bug reports to enhorn@Wirtschaft.tu-ilmenau.de.");
    tty_putstr(BASE_WINDOW, 0, "NetHack[日本語版], Copyright 1993 - 1996");
    tty_putstr(BASE_WINDOW, 0, "         By I. Numata , S. Miyashita and N. Hamada.");
#endif
    tty_putstr(BASE_WINDOW, 0, "JSlash-0.1.3, Copyright 1997 - 2000");
    tty_putstr(BASE_WINDOW, 0, "         By T.Ooi and T.Kihara.");
...
#endif
/*JP	newwin->offy = min((int)ttyDisplay->rows-2, ROWNO+1);*/
...
#ifdef TEXTCOLOR
    int	    color;
    static char colorful_trap[] = { CLR_BROWN,          /* nothing */
				    CLR_GREEN,          /* arrow trap */
				    CLR_CYAN,           /* dart trap */
				    CLR_GRAY,           /* falling rocks */
				    CLR_BROWN,          /* squeaky board */
				    HI_SILVER,          /* beartrap */
				    CLR_GREEN,          /* land mine */
				    CLR_GRAY,           /* rolling boulder trap */
				    CLR_ORANGE,         /* sleep gas */
				    CLR_BLUE,           /* rust trap */
				    CLR_RED,            /* fire trap */
				    CLR_GRAY,           /* pit */
				    CLR_GRAY,           /* spiked pit */
				    CLR_BLACK,          /* hole */      
				    CLR_BLACK,          /* trapdoor */
				    CLR_YELLOW,         /* teleportation */
				    CLR_MAGENTA,        /* level teleporter */
				    CLR_WHITE,          /* magic portal */
				    CLR_GRAY,           /* web */
				    CLR_GRAY,           /* statue trap */
				    CLR_BRIGHT_BLUE,    /* magic trap */
				    CLR_BRIGHT_MAGENTA, /* anti-magic */
				    CLR_BRIGHT_GREEN }; /* polymorph */
...
#define pet_color(c)
	...
	ch = showsyms[offset];
#ifdef TEXTCOLOR
	if (glyph_is_trap(offset)) color = colorful_trap[offset];
	else if (offset != S_altar) cmap_color(offset);
	else {
	    if (a_align(x,y) == A_LAWFUL) color = CLR_WHITE;
	     else if (a_align(x,y) == A_NEUTRAL) color = CLR_GRAY;
	     else if (a_align(x,y) == A_CHAOTIC) color = CLR_BLACK;
	     else if (a_align(x,y) == A_NONE) color = CLR_RED;
	}
#endif
/*  } else if ((offset = (glyph - GLYPH_TRAP_OFF)) >= 0) {       * trap *
	ch = (offset == WEB) ? showsyms[S_web] : showsyms[S_trap];
	color = colorful_trap[offset];
 *      trap_color(offset); */
