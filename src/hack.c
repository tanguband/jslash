...
/*JP		  You("displaced %s.", mtmp->mnamelth ? NAME(mtmp) : mon_nam(mtmp));*/
		  You("%sをずらした．", mtmp->mnamelth ? NAME(mtmp) : mon_nam(mtmp));
		  goto finish_move;
		}
...
/*JP		    You("enter an opulent throne room!");*/
		...
		case REALZOO:
		    pline("すごい匂いがする！");
/*JP		    You("enter a smelly zoo!");*/
		    You("動物園に入った！");
		    break;
		case GIANTCOURT:
/*JP		    You("enter a giant throne room!");*/
		    You("巨大な玉座がある部屋に入った！");
		    break;
		case LEPRECOURT:
/*JP		    You("enter a small throne room!");*/
		    You("小さな玉座がある部屋に入った！");
		    break;
		case DRAGONLAIR:
/*JP		    You("enter a dragon lair...");*/
		    You("ドラゴンの巣に入った．．．");
		    break;
		case COCKNEST:
		    pline("すごい匂いがする！");
/*JP		    You("enter a disgusting nest!");*/
		    ...
		    break;
		case BADFOODSHOP:
/*JP		    You("enter an abandoned store...");*/
		    You("捨てられた店に入った．．．");
		    break;
		case ANTHOLE:
/*JP		    You("enter an anthive!");*/
		    You("アリの巣に入った！");
		    break;
		case MORGUE:
		    if(midnight()) {
...
/*JP		You("die...");*/
	...
	} else if(u.uhp*10 < u.uhpmax && moves-wailmsg > 15 && n > 0){
		wailmsg = moves;
...
/*JP		if (u.uhp < (u.uhpmax/10)) You("are VERY badly hurt.");
		else if (u.uhp < (u.uhpmax/5)) You("are badly hurt.");*/
		if (u.uhp < (u.uhpmax/10)) You("重傷だ．");
		else if (u.uhp < (u.uhpmax/5)) You("酷い傷だ．");
	}
}
