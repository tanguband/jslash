...
static boolean FDECL(shk_offer_price, (char *, int, struct monst *));
...
/*
#ifdef BLACKMARKET
static void FDECL(attack_pet, (register struct monst *));
#endif /* BLACKMARKET */
...
/*JP	Strcpy(kopname, "Keystone Kops");*/
	Strcpy(kopname, "警備員");
...
/*JP		pline("But no one seems to respond to it.");*/
...

	{
		...
		if (flags.verbose)
/*JP		    pline_The("%s appear!", kopname);*/
		    pline("%sが現われた！", kopname);
		mm.x = u.ux;
		mm.y = u.uy;
		makekops(&mm);
		return;
	    ...
	    if (flags.verbose)
/*JP		 pline_The("%s are after you!", kopname);*/
		 pline("%sがいる！", kopname);
...
/*JP	    You("feel like an evil rogue.");*/
	    You("邪悪な盗賊のように感じた．");
	}
...
/*JP	      verbalize("Non-human customers are not welcome!");*/
	      verbalize("人型でないお客さんとは感心しないな！");
	      return;
	    }
#endif /* BLACKMARKET */
...
register boolean silentkops;
{
	...
	if (!Role_is('R')) {
		adjalign(sgn(u.ualign.type));
/*JP		You("feel your guilt vanish.");*/
		You("自分の罪の意識が消えたように感じた．");
	}
	if(!inhishop(shkp)) {
		char shk_nam[BUFSZ];
...
register struct monst *shkp;	/* if angry, impose a surcharge */
{
	...
	if (Is_blackmarket(&u.uz)) {
	  if (obj->oclass==RING_CLASS    || obj->oclass==AMULET_CLASS   ||
	      ...
	      obj->otyp==STONE_OF_HEALTH || obj->otyp==STONE_OF_ROTTING || 
...
register int fall;
{
	...
	if (Role_is('K')) {
	adjalign(-sgn(u.ualign.type));
/*JP	You("feel like a common thief.");*/
	You("こそ泥に成り下がったように感じた．");
	}
	return;
    }
...
coord *mm;
{
	...
	for (ik=0; kop_pm[ik]; ik++) {
	  if (!(mons[kop_pm[ik]].geno & G_EXTINCT)) {
	    while(kop_cnt[ik]--) {
...
/*JP	    return strcpy(buf, shkp ? s_suffix(shkname(shkp)) : "");*/
	    strcpy(buf, shkp ? s_suffix(shkname(shkp)) : "");
	    return strcat(buf, "の");
	}
	return (char *)0;
}
...
static NEARDATA const char weapon_types[] = { WEAPON_CLASS, 0 };
...
/*JP		slang="ugly";*/
		slang="生物さん";
/*JP	else    slang = (flags.female) ? "lady" : "buddy";*/
	slang = (flags.female) ? "お嬢さん" : "お客さん";
	...
	*/
	i = 0;
/*JP	strcpy(svc_buffer, "Services available: ");*/
	strcpy(svc_buffer, "様々なサービスをしています: ");

	/* All shops can identify (some better than others) */
	svc_chars[i++] = 'i';
/*JP	strcat(svc_buffer, "[i]dentify,");*/
	strcat(svc_buffer, "[i]識別，");

	/* All shops can uncurse */
	svc_chars[i++] = 'u';
/*JP	strcat(svc_buffer, " [u]ncurse,");*/
	strcat(svc_buffer, " [u]解呪，");

	/* Weapon appraisals.  Weapon & general stores can do this. */
	if (wizard || shk_class_match(WEAPON_CLASS, shkp))
	{
		svc_chars[i++] = 'a';
/*JP		strcat(svc_buffer, " [a]ppraise,");*/
		strcat(svc_buffer, " [a]鑑定，");
	}
	...
	{
		svc_chars[i++] = 'w';
/*JP		strcat(svc_buffer, " [w]eapon-works,");*/
		strcat(svc_buffer, " [w]武器の改良，");
	}
	...
	{
		svc_chars[i++] = 'r';
/*JP		strcat(svc_buffer, " a[r]mor-works,");*/
		strcat(svc_buffer, " [r]防具の改良，");
	}
	...
	{
		svc_chars[i++] = 'c';
/*JP		strcat(svc_buffer, " [c]harge,");*/
		strcat(svc_buffer, " [c]充填，");
	}
 ...
 */
	...
	/* Pick object */
/*JP	if ( !(obj = getobj(identify_types, "have identified"))) return;*/
	if ( !(obj = getobj(identify_types, "識別します"))) return;

	/* Will shk be guessing? */
	if (guesswork = !shk_obj_match(obj, shkp))
	{
/*JP		pline("I don't handle that sort of item, but I could try...");*/
		pline("その種類のアイテムは扱っていないがやってみましょう．．．");
	}
...
/*      ident_type = yn_function("[B]asic service or [P]remier",
	...
	*/
	if (obj->known && objects[obj->otyp].oc_name_known)
	{
		...
		if (ident_type=='p' && obj->bknown && obj->rknown) ripoff=TRUE;
	}
	else
	{
		ripoff = FALSE;
...
/*JP		pline("%s chuckles greedily...", mon_nam(shkp));*/
		pline("%sは含み笑いをした．．．", mon_nam(shkp));
		mult = 1;
	...
	if (Hallucination)
	{
/*JP		pline("You hear %s tell you it's a pot of flowers.",*/
		pline("%sはあなたにそれは花瓶だと告げた．",
			mon_nam(shkp));
		return;
	}
	else if (Confusion)
	{
/*JP		pline("%s tells you but you forget.", mon_nam(shkp));*/
		pline("%sはあなたに告げたが何を言っていたのか忘れてしまった．", mon_nam(shkp));
		return;
	}
...
/*JP			pline("Success!");*/
		...
		{
/*JP			pline("Sorry.  I guess it's not your lucky day.");*/
			pline("残念だ，思うに今日は運が悪かったようだ．");
			return;
		}
	}

	/* Premier service */
	if (ident_type=='p') obj->dknown=obj->bknown=obj->rknown=TRUE;
	...
	struct monst *shkp;
{
	...
	/* Pick object */
/*JP	if ( !(obj = getobj(identify_types, "uncurse"))) return;*/
	if ( !(obj = getobj(identify_types, "解呪する"))) return;
	...
	{
		/* Not identified! */
/*JP		pline("%s snickers and says \"See, nice and uncursed!\"",*/
		pline("%sはくすくす笑いながら言った「見たところ何処も呪われていないようだ！」",
			mon_nam(shkp));
		obj->bknown = FALSE;
	}
	else if (Confusion)
	{
		/* Curse the item! */
/*JP		You("accidentally ask for the item to be cursed");*/
		You("何を思ったか，アイテムを呪ってくれとせがんだ．");
		obj->cursed = TRUE;
		obj->blessed = FALSE;
	}
	else if (Hallucination)
	{
		...
		*/
		if (rn2(4) == 0)
		{
/*JP		    pline("Distracted by your blood-shot %s, the shopkeeper",*/
		    pline("あなたの血走った%sに注意をそらされた為か，",
			makeplural(body_part(EYE)));
/*JP		    pline("accidentally blesses the item!");*/
		    pline("店主はアイテムを祝福してしまった！");
			obj->cursed = FALSE;
			obj->blessed = TRUE;
		}
		else
		{
/*JP			You("can't see straight and point to the wrong item");*/
			You("視覚がおかしい為違うアイテムを指してしまった．");
		}
	}
	else
	{
/*JP		verbalize("All done - safe to handle, now!");*/
		verbalize("完了だ，もう安全に扱えるよ！");
		obj->cursed = FALSE;
	}
}
...
/*JP	"Basic damage against small foes %s, against large foes %s.";*/
	"小さい敵には%sポイント，大きい敵には%sポイントの基本ダメージを与えます．";
 ...
 */
	...
	/* Pick object */
/*JP	if ( !(obj = getobj(weapon_types, "appraise"))) return;*/
	if ( !(obj = getobj(weapon_types, "鑑定する"))) return;
	...
	if ( ! (obj->known && objects[obj->otyp].oc_name_known) )
	{
/*JP		verbalize("This weapon needs to be identified first!");*/
		verbalize("この武器はまず識別する必要がある！");
		return;
	}
	else if (shk_class_match(WEAPON_CLASS, shkp) == SHK_MATCH)
	{
/*JP		verbalize("Ok, %s, let's see what we have here.", slang);*/
		verbalize("よし，%s，我々が何を持っているか見てみようか．", slang);
		guesswork = FALSE;
	}
	else
	{
/*JP		verbalize("Mind you, I'm not an expert in this field.");*/
		verbalize("始めにめ言っておくが，私はこの分野の専門ではないんだ．");
		guesswork = TRUE;
	}
	...
	if (Confusion)
	{
/*JP		pline("The numbers get all mixed up in your head.");*/
		pline("記憶がゴチャ混ぜになってしまった．");
		return;
	}
	else if (Hallucination)
	{
/*JP		You("hear %s say it'll \"knock 'em dead\"",*/
		You("「それは『死の宣告』だろう」と%sが言ったように聞こえた．",
			mon_nam(shkp));
		return;
	}
	...
	if (guesswork)
	{
		switch (rn2(10))
		{
		    case 1:
			/* Shkp's an idiot */
/*JP			pline("Sorry, %s, but I'm not certain.", slang);*/
			pline("すまない%s，はっきりとは判らないよ．", slang);
			break;

		    case 2:
			/* Not sure about large foes */
/*JP			pline(basic_damage, ascii_wsdam, "?");*/
			pline(basic_damage, ascii_wsdam, "不確定な");
			break;

		    case 3:
			/* Not sure about small foes */
/*JP			pline(basic_damage, "?", ascii_wldam);*/
			pline(basic_damage, ascii_wsdam, "不確定な");
			break;

		    default:
			pline(basic_damage, ascii_wsdam, ascii_wldam);
			break;
...
*/
/*JPstatic const char we_offer[] = "We offer the finest service available!";*/
static const char we_offer[] = "私達は優れたサービスを提供しています！";
static void
shk_weapon_works(slang, shkp)
	char *slang;
	struct monst *shkp;
{
	...
	/* Pick weapon */
/*JP	if ( !(obj = getobj(weapon_types, "improve"))) return;*/
	if ( !(obj = getobj(weapon_types, "改良する"))) return;

	/* Can object be poisoned? */
	if (obj->otyp <= BEC_DE_CORBIN)
	{
/*JP		yn_string = "[r]ust/fireproof, [e]nchant, or [p]oison";*/
		yn_string = "[r]さび/火の耐性を付ける, [e]魔法を掛ける, [p]毒を塗る";
		yn_allows = "rep";
	}
	else
	{
/*JP		yn_string = "[r]ust/fireproof, or [e]nchant";*/
		yn_string = "[r]さび/火の耐性を付ける, [e]魔法を掛ける";
		yn_allows = "re";
	}
	...
	{
	    case 'r':
/*JP		verbalize("This'll leave your %s untouchable!", xname(obj));*/
		verbalize("このサービスはあなたの%sを錆びなくします！", xname(obj));
		
		/* Costs more the more rusty it is (oeroded 0-3) */
		charge = 500 * (obj->oeroded+1);
		if (obj->oeroded > 2)
/*JP			pline("This thing's in pretty sad condition, %s",*/
			pline("これはとても酷い状態ですね，%s",
				slang);
		...
		if (Confusion)
/*JP			You("fall over in appreciation");*/
			You("評価を落した．");
		else if (Hallucination)
/*JP			Your(" - tin roof, un-rusted!");*/
			Your(" - tin roof，錆びなくなった！");
	    ...
	    case 'e':
/*JP		verbalize("Guaranteed not to harm your weapon, or your money back!");*/
		verbalize("武器に不都合があった場合は全額返済することを保証します！");
		...
		*/
		...
		if (shk_offer_price(slang, charge, shkp) == FALSE) return;
		if (obj->spe+1 > 5) { 
/*JP			pline("I can't enchant this any higher!");*/
			pline("私にはこれ以上魔法を掛けることができません！");
			charge = 0;
			break;
			}
		/* Have some fun! */
		if (Confusion)
/*JP			Your("%s vibrates unexpectedly!", xname(obj));*/
			Your("%sは激しく振動した！", xname(obj));
		else if (Hallucination)
/*JP			Your("%s evaporates into thin air!", xname(obj));*/
			Your("%sは蒸発して空気に融け込んだ！", xname(obj));
...
/*JP			Your("weapon seems sharper now.");*/
	    ...
	    case 'p':
/*JP		verbalize("Just imagine what a poisoned %s can do!",*/
		verbalize("毒の塗られた%sで何ができるかちょっと考えてみてくれ！",
			xname(obj));
	...
	struct monst *shkp;
{
	...
	/* Pick weapon */
/*JP	if ( !(obj = getobj(armor_types, "improve"))) return;*/
	if ( !(obj = getobj(armor_types, "改良する"))) return;

	/* Here we go */
/*JP	c=yn_function("[r]ust/fireproof, or [e]nchant", "re", '\0');*/
	c=yn_function("[r]さび/火の耐性を付ける, [e]魔法を掛ける", "re", '\0');
	verbalize(we_offer);

	switch(c)
	{
	    case 'r':
		if (!flags.female && is_human(uasmon))
/*JP		     verbalize("They'll call you the man of stainless steel!");*/
		     verbalize("皆があなたを錆びない鋼の男と呼ぶでしょう！");

		/* Costs more the more rusty it is (oeroded 0-3) */
		charge = 300 * (obj->oeroded+1);
/*JP		if (obj->oeroded > 2) pline("Yikes!  This thing's a mess!");*/
		if (obj->oeroded > 2) pline("おゎ！これは面倒だ！");
		...
		/* Have some fun, but for this $$$ it better work. */
		if (Confusion)
/*JP			You("forget how to put your %s back on!", xname(obj));*/
			You("%sをどうやって元に戻すか忘れた！", xname(obj));
		else if (Hallucination)
/*JP			You("mistake your %s for a pot and...", xname(obj));*/
			You("を%sおまると間違え，そして．．．", xname(obj));
	    ...
	    case 'e':
/*JP		verbalize("Nobody will ever hit on you again.");*/
		verbalize("もう誰もあなたに打撃を与えることは無いだろう．");
...
/*JP			pline("I can't enchant this any higher!");*/
		 ...
		 /* Have some fun! */
/*JP		if (Hallucination) Your("%s looks dented.", xname(obj));*/
		if (Hallucination) Your("%sはへこんだようだ．", xname(obj));
		
		if (obj->otyp >= GRAY_DRAGON_SCALES &&
					obj->otyp <= YELLOW_DRAGON_SCALES) {
			/* dragon scales get turned into dragon scale mail */
/*JP			Your("%s merges and hardens!", xname(obj));*/
...
/*JP		obj = getobj(wand_types, "charge");*/
		obj = getobj(wand_types, "充填する");
	else if (shk_class_match(TOOL_CLASS, shkp) == SHK_MATCH)
/*JP		obj = getobj(tool_types, "charge");*/
		obj = getobj(tool_types, "充填する");
	else if (shk_class_match(RING_CLASS, shkp) == SHK_MATCH)
/*JP		obj = getobj(ring_types, "charge");*/
		obj = getobj(ring_types, "充填する");
	if (!obj) return;
...
/*JP		type = yn_function("[B]asic service or [P]remier",*/
		type = yn_function("[B]一般サービス，[P]特別サービス",
			ident_chars, '\0');
		if (!type) return;
	}
	else
	...
	{
/*JP		pline("%s says it's charged and pushes you toward the door",*/
		pline("%sは充填完了と言ってあなたをドアの向うに押しやった．",
			mon_nam(shkp));
		return;
	}
	...
	*/
	...
	if (!obj)
	{
/*JP		verbalize("Oops!  Sorry about that...");*/
		verbalize("おっと！それはちょっと出来ないな．．．");
		return;
	}
	...
	if (obj->oclass==WAND_CLASS)
	{
		...
		if (obj->otyp==WAN_WISHING)
		{
			...
			/* Fun */
/*JP		verbalize("Since you'll have everything you always wanted,");
			verbalize("...How about loaning me some money?");*/
		verbalize("何時でも欲しい時にあらゆる物が手に入るので．．．");
			verbalize("どれだけお金を投資してくれます？");
			shkp->mgold += u.ugold;
			u.ugold = 0;
			bot();
		}
	...
	struct monst *shkp;
{
	/* There must be a better way.  Need nh data structures 101. */
        const struct shclass *shp = &shtypes[ESHK(shkp)->shoptype-SHOPBASE];
...
/*JP	Sprintf(sbuf, "It'll cost you %ld zorkmid%s.  Interested?",
		charge, plur(charge));*/
	Sprintf(sbuf, "%ldゴールドかかるがそれでいいかい？",charge);

	if ( yn(sbuf) != 'y' )
	{
/*JP		verbalize("It's your call, %s.", slang);*/
		verbalize("それはあなたの請求次第だよ，%s．", slang);
		return(FALSE);
	}
	...
	if (charge > u.ugold)
	{
/*JP		verbalize("Cash on the spot, %s, and you ain't got the dough!",*/
		verbalize("%s，料金を支払ってくれ，じゃないと品物は返せないな！",
			slang);
		return(FALSE);
	}
	...
	int upper;
{
	...
	int bonus;
	     ...
	     if(ACURR(A_CHA) > 21) *pcharge *= 1.1;
	else if(ACURR(A_CHA) > 18) *pcharge *= 1.2;
	else if(ACURR(A_CHA) > 15) *pcharge *= 1.3;
	else if(ACURR(A_CHA) > 12) *pcharge *= 1.4;
	else if(ACURR(A_CHA) > 10) *pcharge *= 1.5;
	else if(ACURR(A_CHA) > 8)  *pcharge *= 1.6;
	else if(ACURR(A_CHA) > 7)  *pcharge *= 1.7;
	else if(ACURR(A_CHA) > 6)  *pcharge *= 1.8;
	else if(ACURR(A_CHA) > 5)  *pcharge *= 1.9;
	else if(ACURR(A_CHA) > 4)  *pcharge *= 2.0;
	else *pcharge *= 2.1;
...
/* wake up assistants */
/*JP  pline("%s calls for his assistants!", Monnam(shkp));*/
  pline("%sは店員を招集した！", Monnam(shkp));
