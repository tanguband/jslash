			...
			if(!rn2(u.uevent.udemigod ? 25 :
				(depth(&u.uz) >
				 depth(&stronghold_level))
						? 50 : 70)) {
#ifdef BLACKMARKET                            
/* random monster creation disabled in black market */
			...
			if (u.usacrifice && !(moves%250)) {                    
				u.usacrifice--;
#ifdef WIZARD
				if (wizard) pline("Sacrifice: %d", u.usacrifice);
#endif
			}                        
 ...
 * [Tom] overhauled the healing code
 * with a CON of 25, heal 0.25% per turn. others proportional.

   stone of healing:  x1.5
   stone of rotting:  divided by 1.5
   regeneration:      x2.5

  also, since release #3 of slash, made hp heal 1.5 times faster. :)
 ...
 */
		        	...
		        	int per_turn;
		        	float remainder;
		        	healpercent = (float)(ACURR(A_CON)+25);
		        	healpercent /= 75.0;
		        	if (HRegeneration) healpercent *= 2.5;
		        	if (carrying(STONE_OF_HEALTH)) healpercent *= 1.5;
		        	if (carrying(STONE_OF_ROTTING)) healpercent /= 1.5;
		        	if (healpercent < 0.05) healpercent = 0.05;
		        	/* convert to true percentage */
		        	healpercent /= 100.0;
		        	/* how many points you heal a turn (if 1 or greater) */
		        	heal_turn = (int)((float)u.uhpmax * healpercent);
		        	if (heal_turn < 1) {
				/* less than 1 point a turn */
		        	/* over 1000 turns, how many points do you heal? */
		        		total_points = (int)(u.uhpmax * healpercent * 1000);
		        		turn_interval = (int)(1000/total_points);
		        		if (!(moves%turn_interval)) {
						u.uhp++;
				flags.botl = 1;
					}                            
				} else {
					per_turn = (int)(heal_turn+0.5);
					remainder = (((float)u.uhpmax * healpercent) - (float)per_turn );
			    flags.botl = 1;
					u.uhp += per_turn;                        
					total_points = (int)(remainder * 1000);
					if(total_points){
						turn_interval = (int)(1000/total_points);
						if (!(moves%turn_interval)) u.uhp++;
					}
			}
...
/* STEPHEN WHITE'S NEW CODE */                    
			...
			/* [Tom] Dopplegangers don't polymorph unless they                            
				want to... */
			if(Polymorph && !rn2(100) && !Role_is('D')) {
			if (Role_is('D') && rn2(6))
/*JP			    c = yn("Do you want to polymorph?");*/
			    c = yn("変化しますか？");
			else c = 'y';
			if (c == 'y') {
			    if (multi >= 0) {
	...
	if (!rn2(40+(int)(ACURR(A_DEX)*3))) u_wipe_engr(rnd(3));
		...
		if (Is_waterlevel(&u.uz)) movebubbles();
		    else if (Underwater) under_water(0);
		    /* vision while buried done here */
		    else if (u.uburied) under_ground(0);

		if ((oldmtimedone && !Upolyd) ||
			(!oldmtimedone && Upolyd)) moverate = 0;
		}
		if(multi < 0) {
		...
		if(!flags.mv || Blind) {
			...
			(Upolyd && (u.umonnum == PM_MIND_FLAYER ||
			 u.umonnum == PM_FLOATING_EYE)))
			see_monsters();
...
# endif /* REDO */
		...
		if(u.utrap && u.utraptype == TT_LAVA) {
			if(!is_lava(u.ux,u.uy)) u.utrap = 0;
		    else {
			u.utrap -= 1<<8;
			if(u.utrap < 1<<8) {
...
newgame()
{
	...
	u_init();
	/* [Tom] moved down here so Drow quest is initialized right...
