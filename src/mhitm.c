...
/*JP		pline("%s %s.", buf, mon_nam_too(mdef_name, mdef, magr));*/
...
/*JP                        pline("%s %s %s.", buf, mon_nam_too(mdef_name, mdef, magr),
				compat == 2 ?
	...
	register struct attack	*mattk;
{
	...
	if ((pd == &mons[PM_COCKATRICE] ||
	     pd == &mons[PM_CHICKATRICE] ||
	     pd == &mons[PM_BASILISK] ||
	     pd == &mons[PM_ASPHYNX])
	    && !resists_ston(magr) &&
...
/*JP		    You(brief_feeling, "peculiarly sad");*/
	...
	if((mattk->aatyp == AT_WEAP) && (otmp)) {
		...
		if (otmp && (otmp->otyp == LIGHTSABER || otmp->otyp == DARKSABER)) objenchant = 4;
	} else objenchant = 0;
...
/*JP		    pline("%s last thought fades away...",*/
	...
	if (objenchant < canhitmon && vis) {
			Strcpy(buf, Monnam(magr));
/*JP			pline("%s doesn't seem to harm %s.", buf,
								mon_nam(mdef));*/
			pline("%sは%sにダメージを与えることができないようだ．",
						buf, mon_nam(mdef));
		return(MM_HIT);
		}
	if((mdef->mhp -= tmp) < 1) {
