...
/* -----------============STEPHEN WHITE'S NEW CODE============----------- */                   
	case WEAPON_CLASS:
		otmp->quan = (otmp->otyp <= SHURIKEN) ? (long) rn1(10,20) : 1L;
		if(!rn2(11)) {
			otmp->spe = rne(3);
			otmp->blessed = rn2(2);
		} else if(!rn2(10)) {
		...
		if (!rn2(50) && (otmp->otyp <= BEC_DE_CORBIN))
			otmp->opoisoned = 1;
		if (artif && !rn2(20))
...
/* -----------============STEPHEN WHITE'S NEW CODE============----------- */           
	case GEM_CLASS:
		if (otmp->otyp == LOADSTONE || otmp->otyp == 
		STONE_OF_ROTTING) curse(otmp);
		else if (otmp->otyp == ROCK) otmp->quan = (long) rn1(6,6);
		else if (!(otmp->otyp == LUCKSTONE || 
		otmp->otyp == STONE_OF_HEALTH) && !rn2(6)) otmp->quan = 2L;
		else otmp->quan = 1L;
		break;
/* -----------============STEPHEN WHITE'S NEW CODE============----------- */           
	case TOOL_CLASS:
		...
		case BAG_OF_HOLDING:	mkbox_cnts(otmp);
					break;
		case TINNING_KIT:       otmp->spe = rn1(30,70);
					break;
		case MAGIC_MARKER:	otmp->spe = rn1(70,30);
					break;
		case CAN_OF_GREASE:     otmp->spe = rnd(25) + 10;
					blessorcurse(otmp, 10);
					break;
		...
		case ORB_OF_ENCHANTMENT:otmp->spe = rnd(3) + 1;
					blessorcurse(otmp, 2);
					break;
		case CRYSTAL_BALL:      otmp->spe = rnd(10) + 3;
					blessorcurse(otmp, 2);
					break;
		case MEDICAL_KIT:       otmp->spe = rn1(10,10);
					break;                
		case HORN_OF_PLENTY:
		case BAG_OF_TRICKS:     otmp->spe = rnd(20) + 10;
					break;
