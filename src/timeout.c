...
static NEARDATA const char *slime_texts[] = {
	...
	"You have turned into green slime.",         /* 2 */
	"You are a pool of slime."                   /* 1 */
#endif /*JP*/
	"あなたの体は少し緑がかった．",			/* 5 */
	"あなたの体はだらだら流れ始めた．",		/* 4 */
	"あなたの肌がはげて落ち始めた．",		/* 3 */
...
STATIC_OVL void
slime_dialogue() {
	register long i = (Slimed & TIMEOUT) / 3L;

	if ((((Slimed & TIMEOUT) % 3L) == 2) && (i >= 0)
	    && (i < SIZE(slime_texts)))
 ...
 */
	...
	if(Upolyd) if(!--u.mtimedone) rehumanize();
	...
	for(upp = u.uprops; upp < u.uprops+SIZE(u.uprops); upp++)
		...
		case STONED:
			if (!killer) {
				...
				Sprintf( kill_buf, "%sの攻撃で", trnstn_monnam );
				killer = kill_buf;
			} done(STONING);
			break;
		case SLIMED:
			if (!killer) {
				killer_format = KILLED_BY_AN;
/*JP				killer = "green slime";*/
				killer = "グリーンスライムに感染されて";
			} done(TURNED_SLIME);
			break;
		case VOMITING:
...
/*JP				You_feel("yourself slowing down%s.",
		...
		case FREE_ACTION:
			Free_action = 0;
/*JP			You("feel a little stiffer.");*/
			You("ちょっと動きずらくなったような気がした．");
			break;
		case INVULNERABLE:
			Invulnerable = 0;
/*JP			You("are no longer invulnerable.");*/
			You("もう不死身ではない．");
			break;
		case FIRE_RES:
			if (Fire_resistance & ~INTRINSIC)
				;
			else {
			HFire_resistance = 0;
/*JP			You("feel a little warmer.");*/
			...
			break;
		case COLD_RES:
			if (Cold_resistance & ~INTRINSIC)
				;
			else {
			HCold_resistance = 0;
/*JP			You("feel a little cooler.");*/
			You("少し冷たく感じた．");
			}
			break;
		case SLEEP_RES:
			if (Sleep_resistance & ~INTRINSIC)
				;
			else {
			HSleep_resistance = 0;
/*JP			You("feel a little sleepy.");*/
			You("少し眠気がした．");
			}
			break;
		case SHOCK_RES:
			if (Shock_resistance & ~INTRINSIC)
				;
			else {
			HShock_resistance = 0;
/*JP			You("feel a little static cling.");*/
			You("少し静電気を感じた．");
			}
			break;
		case POISON_RES:
			if (Poison_resistance & ~INTRINSIC)
				;
			else {
			HPoison_resistance = 0;
/*JP			You("feel a little less healthy.");*/
			You("少し健康的でなくなったような気がした．");
			}
			break;
		case DISINT_RES:
			if (Disint_resistance & ~INTRINSIC)
				;
			else {
			HDisint_resistance = 0;
/*JP			You("feel a little less firm.");*/
			You("少し頑丈でなくなったような気がした．");
			}
			break;
		case TELEPORT:
			if (Teleportation & ~INTRINSIC)
				;
			else {
			HTeleportation = 0;
/*JP			You("feel a little less jumpy.");*/
			You("少し跳躍力が衰えたような気がした．");
			}
			break;
		case TELEPORT_CONTROL:
			if (Teleport_control & ~INTRINSIC)
				;
			else {
			HTeleport_control = 0;
/*JP			You("feel a little less in control of yourself.");*/
			You("少し制御力を失ったような気がした．");
			}
			break;
		case TELEPAT:
			if (Telepat & ~INTRINSIC) /* helmet */
				;
			else {
			  HTelepat = 0;
/*JP			  You("feel a little less mentally acute.");*/
			  You("少し精神的鋭さを失ったような気がした．");
			}
			break;
		case CONFUSION:
...
/*JP				You("are no longer invisible.");*/
		...
		case SLEEPING:
			if (unconscious() || Sleep_resistance)
				...
				if(uamul && uamul->otyp ==
				   AMULET_OF_RESTFUL_SLEEP && uamul->blessed)
					break;
/*JP				You("fall asleep.");*/
				You("眠りに落ちた．");
				sleeptime = rnd(20);
...
const char *tailer;
{
	switch (obj->where) {
	    case OBJ_INVENT:
	    case OBJ_MINVENT:
/*JP		pline("%s flickers%s.", Yname2(obj), tailer);*/
		pline("%sは%s点滅した．", Yname2(obj), tailer);
		break;
	    case OBJ_FLOOR:
/*JP		You("see %s flicker%s.", an(xname(obj)), tailer);*/
	        You("%sは%s点滅した．", an(xname(obj)), tailer);
		break;
	}
}
