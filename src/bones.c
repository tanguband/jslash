...
can_make_bones()
{
	...
	if(depth(&u.uz) <= 1) return FALSE;     /* bulletproofing for endgame */
 /*        (!rn2(2) && rn2(depth(&u.uz)))       /* fewer ghosts on low levels */
	/* [Tom] changed to flat 50% bones */
	/* changed back to varying by depth */
	if (!rn2(2) && rn2(depth(&u.uz))
#ifdef WIZARD
		&& !wizard
#endif
		) return FALSE;
