...
struct mkroom *sroom;
{
	...
	switch(type) {
		...
		if (type == LEPRECOURT) goldlim = 500 * level_difficulty();
	...
	for(sx = sroom->lx; sx <= sroom->hx; sx++)
		...
		if(type == COURT && IS_THRONE(levl[sx][sy].typ))
		    continue;
		switch (type) {                
		    case ZOO: mon = makemon(0,sx,sy, NO_MM_FLAGS);
		    break;
		    case COURT: mon = makemon(courtmon(),sx,sy, NO_MM_FLAGS);
		    break;
		    case ANTHOLE: mon = makemon(antholemon(),sx,sy, NO_MM_FLAGS);
		    break;
		    case BARRACKS: mon = makemon(squadmon(),sx,sy, NO_MM_FLAGS);
		    break;
		    case BADFOODSHOP: mon = makemon(mkclass(S_BAD_FOOD,0),sx,sy, NO_MM_FLAGS);
		    break;
		    case DRAGONLAIR: mon = makemon(mkclass(S_DRAGON,0),sx,sy, NO_MM_FLAGS);
		    break;
		    case COCKNEST:
			if (!rn2(4)) mon = makemon(&mons[PM_COCKATRICE],sx,sy, NO_MM_FLAGS);
			else mon = makemon(&mons[PM_CHICKATRICE],sx,sy, NO_MM_FLAGS);
		    break;
		    case REALZOO: mon = makemon(realzoomon(),sx,sy, NO_MM_FLAGS);
		    break;
		    case LEPRECOURT: mon = makemon(mkclass(S_LEPRECHAUN,0),sx,sy, NO_MM_FLAGS);
		    break;
		    case GIANTCOURT: mon = makemon(mkclass(S_GIANT,0),sx,sy, NO_MM_FLAGS);
		    break;
		    case MORGUE: mon = makemon(morguemon(),sx,sy, NO_MM_FLAGS);
		    break;
		    case BEEHIVE:
			if ((sx == tx) && (sy == ty)) mon = makemon(&mons[PM_QUEEN_BEE],sx,sy, NO_MM_FLAGS);
			else mon = makemon(&mons[PM_KILLER_BEE],sx,sy, NO_MM_FLAGS);
		    break;
		}
		if(mon) {
			mon->msleep = 1;
		...
		switch(type) {
		    ...
		    case COCKNEST:
			if(!rn2(3))
			    (void) mk_tt_object(STATUE, sx, sy);
			for (i=0;i<rn2(5);i++) {
...
{
	int     i = rn2(60) + rn2(3*level_difficulty());
	if (i > 75)     return(&mons[PM_ARMY_ANT]);
	else if (i > 55)        return(&mons[PM_SNOW_ANT]);
	else if (i > 40)        return(&mons[PM_FIRE_ANT]);
	else if (i > 25)        return(&mons[PM_SOLDIER_ANT]);
