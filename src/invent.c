...
/*JP		|| (!strcmp(word, "sacrifice") &&*/
		|| ((!strcmp(word, "sacrifice")||!strcmp(word,"捧げる")) &&
		    (otyp != CORPSE &&
		     otyp != ART_EYE_OF_VECNA &&                    
		     otyp != ART_HAND_OF_VECNA &&
...
boolean force_touch;
{
        if((Blind || force_touch)  && !uarmg && !resists_ston(&youmonst) &&
...
/*JP	    instapetrify("cockatrice corpse");*/
        ...
        if((Blind || force_touch) && !uarmg && !resists_ston(&youmonst) &&
	    (otmp->otyp == CORPSE && otmp->corpsenm == PM_CHICKATRICE)) {
	    if(poly_when_stoned(uasmon)) {
/*JP		You("touched the chickatrice corpse with your bare hands.");*/
		You("シカトリスの死体に素手で触った．");
		(void) polymon(PM_STONE_GOLEM);
	    } else {
/*JP		  pline("Touching the chickatrice corpse is a fatal mistake...");
		  You("turn to stone...");*/
		  pline("シカトリスの死体に触れるのは致命的な間違いだ．．．");
		  You("石になった．．．");
		  killer_format = KILLED_BY_AN;
/*JP		  killer = "chickatrice corpse";*/
		  killer = "シカトリスの死体に触れて";
		  done(STONING);
		   }
	    }
            if((Blind || force_touch) && !uarmg && !resists_ston(&youmonst) &&
		(otmp->otyp == CORPSE && otmp->corpsenm == PM_BASILISK)) {
		if(poly_when_stoned(uasmon)) {
/*JP		   You("touched the basilisk corpse with your bare hands.");*/
		   You("バジリスクの死体に素手で触った．");
		   (void) polymon(PM_STONE_GOLEM);
		} else {
/*JP		      pline("Touching the basilisk corpse is a fatal mistake...");
		      You("turn to stone...");*/
		      pline("バジリスクの死体に触れるのは致命的な間違いだ．．．");
		      You("石になった．．．");
		      killer_format = KILLED_BY_AN;
/*JP		      killer = "basilisk corpse";*/
		      killer = "バジリスクの死体に触れて";
		      done(STONING);
		}
	    }
            if((Blind || force_touch) && !uarmg && !resists_ston(&youmonst) &&
		(otmp->otyp == CORPSE && otmp->corpsenm == PM_ASPHYNX)) {
		if(poly_when_stoned(uasmon)) {
/*JP		    You("touched the asphynx corpse with your bare hands.");*/
		    You("アズフィンクスの死体に素手で触った．");
		    (void) polymon(PM_STONE_GOLEM);
		} else {
/*JP		      pline("Touching the asphynx corpse is a fatal mistake...");
		      You("turn to stone...");*/
		      pline("アズフィンクスの死体に触れるのは致命的な間違いだ．．．");
		      You("石になった．．．");
		      killer_format = KILLED_BY_AN;
/*JP		      killer = "asphynx corpse";*/
		      killer = "アズフィンクスの死体に触れて";
		      done(STONING);
		}
	    }
}
