...
/*JP	      pline("%s tries to rob you, but there is nothing to steal!",*/
	...
	for(otmp = invent; otmp; otmp = otmp->nobj)
	    ...
	    && (otmp->oinvis == 0 || perceives(mtmp->data)))
		tmp += ((otmp->owornmask &
...
gotobj:
	...
	if((otmp->owornmask & (W_ARMOR | W_RING | W_AMUL | W_TOOL))){
		...
		case ARMOR_CLASS:
			...
			otmp->cursed = 0;
			stop_occupation();
/* [Tom] made stealing message more generic for thieves, etc. */
if ((flags.female) || (mtmp->data->mlet != S_NYMPH))
/*JP			    pline("%s manages to %s your %s!",
				  Blind ? "It" : Monnam(mtmp),
				  curssv ? "remove" : "take off",
				  equipname(otmp));*/
			    pline("%sはあなたの%sをうまく%s！",
...
/*JP			    pline("%s seduces you and %s off your %s.",
	...
	else if (otmp == uquiver) setuqwep((struct obj *) 0);
	else if (otmp == uswapwep) setuswapwep((struct obj *) 0);

	if(otmp == uball) unpunish();

	freeinv(otmp);
/*JP	pline("%s stole %s.", named ? "It" : Monnam(mtmp), doname(otmp));*/
	pline("%sは%sを盗んだ", named ? "そいつ" : Monnam(mtmp), doname(otmp));
	mpickobj(mtmp,otmp);
