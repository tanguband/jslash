...
#ifdef BLACKMARKET
static void FDECL(stock_blkmar, 
...
/*JP	{"general store", RANDOM_CLASS, 24,*/
	{"ª®≤ﬂ≤∞", RANDOM_CLASS, 24,
	    D_SHOP, {{100, RANDOM_CLASS}, {0, 0}, {0, 0}}, shkgeneral},
/*JP	{"used armor dealership", ARMOR_CLASS, 20,*/
	{"√Ê∏≈≥ªæ¶≤Ò", ARMOR_CLASS, 20,
	    D_SHOP, {{90, ARMOR_CLASS}, {10, WEAPON_CLASS}, {0, 0}},
	     shkarmors},
/*JP	{"second-hand bookstore", SCROLL_CLASS, 4, D_SHOP,*/
	{"¥¨ ™≤∞", SCROLL_CLASS, 4, D_SHOP,
	    {{90, SCROLL_CLASS}, {10, SPBOOK_CLASS}, {0, 0}}, shkbooks},
/*JP	{"liquor emporium", POTION_CLASS, 4, D_SHOP,*/
	{"§™ºÚ§Œ…¥≤ﬂ≈π", POTION_CLASS, 4, D_SHOP,
	    {{100, POTION_CLASS}, {0, 0}, {0, 0}}, shkliquors},
/*JP	{"antique weapons outlet", WEAPON_CLASS, 20, D_SHOP,*/
	{"∏≈…¥Ô¿ÏÃÁæÆ«‰≈π", WEAPON_CLASS, 20, D_SHOP,
	    {{90, WEAPON_CLASS}, {10, ARMOR_CLASS}, {0, 0}}, shkweapons},
/*JP	{"delicatessen", FOOD_CLASS, 4, D_SHOP,*/
	{"ø©… ≈π", FOOD_CLASS, 4, D_SHOP,
	    {{95, FOOD_CLASS}, {5, POTION_CLASS}, {0, 0}}, shkfoods},
/*JP	{"jewelers", RING_CLASS, 4, D_SHOP,*/
	{" ı¿–≈π", RING_CLASS, 4, D_SHOP,
	    {{85, RING_CLASS}, {10, GEM_CLASS}, {5, AMULET_CLASS}, {0, 0}},
	    shkrings},
/*JP	{"quality apparel and accessories", WAND_CLASS, 4, D_SHOP,*/
	{"§™•∑•„•Ï§ ÕŒ… ≈π", WAND_CLASS, 3, D_SHOP,
	    {{90, WAND_CLASS}, {5, -LEATHER_GLOVES}, {5, -ELVEN_CLOAK}, {0, 0}},
	     shkwands},
/*JP	{"hardware store", TOOL_CLASS, 4, D_SHOP,*/
	{"∆ª∂Ò≤∞", TOOL_CLASS, 4, D_SHOP,
	    {{100, TOOL_CLASS}, {0, 0}, {0, 0}}, shktools},
	/* STEPHEN WHITE'S NEW CODE */        
/*JP	{"pet store", TOOL_CLASS, 8, D_SHOP,*/
	{"•⁄•√•»•∑•Á•√•◊", TOOL_CLASS, 8, D_SHOP,
	 ...
	 * random shopkeepers)
	 */
/*JP	{"rare books", SPBOOK_CLASS, 4, D_SHOP,*/
	{"∏∏§ŒÀ‹≤∞", SPBOOK_CLASS, 4, D_SHOP,
...
/*JP	{"lighting store", TOOL_CLASS, 0, D_SHOP,*/
	{"æ»Ã¿≈π", TOOL_CLASS, 0, D_SHOP,
	    {{25, -WAX_CANDLE}, {40, -TALLOW_CANDLE},{11, -BRASS_LANTERN},             
	    {16, -OIL_LAMP}, {3, -MAGIC_LAMP}, {5, -MAGIC_CANDLE}}, shktools},
#ifdef BLACKMARKET
	{"•÷•È•√•Ø•ﬁ°º•±•√•»", RANDOM_CLASS, 0, D_SHOP,
...
register int sh;
{
     ...
     * door get objects).
     */
    register int i, sx, sy, cl, first=0, next=0, total, partial, typ;
    char buf[BUFSZ];
    struct obj *otmp;
