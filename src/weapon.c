...
/*	Put weapon specific "to hit" bonuses in below:		*/
	...
	if (u.twoweap)
		tmp += ((P_SKILL(P_TWO_WEAPON_COMBAT)-2) * 2) - 4;
#endif /* WEAPON_SKILLS */
...
int x;
{
	...
	for (otmp = mtmp->minvent; otmp; otmp = otmp->nobj) {
	    if (otmp->otyp == x &&
		    ...
		    !(x == CORPSE && !(TURNSTONE(otmp->corpsenm))) &&
...
static NEARDATA const int rwep[] =
	{ DWARVISH_SPEAR, ELVEN_SPEAR, SPEAR, ORCISH_SPEAR, SILVER_SPEAR,
	  JAVELIN, SHURIKEN, SILVER_ARROW, ELVEN_ARROW, ARROW, ORCISH_ARROW,
	  CROSSBOW_BOLT, ELVEN_DAGGER, DAGGER, ORCISH_DAGGER, KNIFE, ROCK,
	  LOADSTONE, LUCKSTONE, SILVER_DAGGER, DART, /* BOOMERANG, */ CREAM_PIE
...
register struct monst *mtmp;
{
	...
	for (i = 0; i < SIZE(rwep); i++) {
		...
		for(otmp=mtmp->minvent; otmp; otmp=otmp->nobj) {
		    ...
		    if(otmp->oclass==GEM_CLASS && otmp->otyp != LOADSTONE && 
		      !otmp->cursed) return(otmp);
		}
	    }
...
/* STEPHEN WHITE'S NEW CODE */                                                
		    ...
		    otmp->otyp == STONE_OF_ROTTING) && !otmp->cursed)
			return otmp;
		}
	    }
	  }
...
static NEARDATA short hwep[] = {
	  ...
	  TRIDENT, LONG_SWORD, ELVEN_BROADSWORD, BROADSWORD, 
	  LUCERN_HAMMER, SCIMITAR, SILVER_SABER, SILVER_DAGGER,
	  SILVER_SHORT_SWORD, SILVER_LONG_SWORD, SILVER_MACE,
	  SILVER_SPEAR, HALBERD, PARTISAN, LANCE, 
	  FAUCHARD, BILL_GUISARME, BEC_DE_CORBIN, GUISARME, 
	  RANSEUR, SPETUM, VOULGE, BARDICHE, MORNING_STAR, 
	  GLAIVE, ELVEN_SHORT_SWORD, DWARVISH_SHORT_SWORD, 
	  SHORT_SWORD, ORCISH_SHORT_SWORD, MACE, AXE, 
	  DWARVISH_SPEAR, ELVEN_SPEAR, SPEAR, ORCISH_SPEAR, 
	  FLAIL, BULLWHIP, QUARTERSTAFF, JAVELIN, AKLYS, CLUB,
	  PICK_AXE,
#ifdef KOPS
	  RUBBER_HOSE,
#endif /* KOPS */
...
abon()		/* attack bonus for strength & dexterity */
{
	...
	else if (str < 118) sbon = 1;   /* up to 18/99 */        
	else if (str == 118) sbon = 2;  /* 18/00 */
	else if (str == 119) sbon = 2;  /* 19 */
	else if (str == 120) sbon = 3;  /* 20 */
	else if (str == 121) sbon = 3;  /* 21 */
	else if (str == 122) sbon = 4;  /* 22 */
	else if (str == 123) sbon = 4;  /* 23 */
	else if (str == 124) sbon = 5;  /* 24 */
	else sbon = 5;
...
dbon()		/* damage bonus for strength */
{
	...
	else if (str < 109) return(3);          /* up to 18/90 */        
	else if (str == 118) return(4);         /* 18/00 */
	else if (str == 119) return(5);         /* 19 */
	else if (str == 120) return(6);         /* 20 */
	else if (str == 121) return(7);         /* 21 */
	else if (str == 122) return(8);         /* 22 */
	else if (str == 123) return(9);         /* 23 */
	else if (str == 124) return(10);         /* 24 */
	else return(11);
}
...
enhance_weapon_skill()
{
	...
	/*if (i == P_TWO_WEAPON_COMBAT) continue;	/* work now */
	/*
...
/*JPStrcpy(buf, to_advance ? "Pick a skill to advance:" : "Current skills:");*/
