...
/*JP			pline("Yecch! Your %s %s gunk on it!", body_part(FACE),
			...
			pline("ゲェー！あなたの%sは%sべとべとになった！",
			      body_part(FACE), (old ? "もっと" : ""));
...
/*JP	       pline("%s smiles seductively at the sight of this prop!", Monnam(mtmp));*/
	       pline("%sはその道具を見て魅惑的に微笑んだ！", Monnam(mtmp));
	       mtmp->mtame = 10;
	       mtmp->mpeaceful = 1;
	       set_malign(mtmp);
	}
...
/*JP	       pline("%s looks shocked! \"I'm not that kind of girl!\"", Monnam(mtmp));*/
	       pline("%sはショックを受けたようだ！", Monnam(mtmp));
	       pline("「私はそんな娘じゃないわ！この変態！！」");
	       mtmp->mtame = 0;
	       mtmp->mpeaceful = 0;
	       mtmp->msleep = 0;
	}
...
/*JP		You("remove the leash from %s%s.",
		...
		if ((mtmp->data == &mons[PM_SUCCUBUS]) || (mtmp->data == &mons[PM_INCUBUS]))
		{
/*JP		    pline("%s is infuriated!", Monnam(mtmp));*/
		    pline("%sは激怒した！", Monnam(mtmp));
		    mtmp->mtame = 0;
		    mtmp->mpeaceful = 0;
		}
...
/*JP			    pline("You stiffen momentarily under your gaze.");*/
			    pline("自分の睨みで一瞬硬直した．");
			}
...
/*JP		if (mon_reflects(mtmp, "The gaze is reflected away by %s %s!"))*/
		if (mon_reflects(mtmp, "眈みは%sの%sで反射した！"))
			return 1;
		if (vis)
...
dig_up_grave(void)
{
    int num_obj;
    int o;
    if (pl_character[0] == 'A') { /* grave-robbing! */
	    adjalign(-sgn(u.ualign.type)*3);
/*JP	    You("feel like a despicable grave-robber!");*/
	    You("卑劣な墓荒しの様に感じた！");
    } else if (pl_character[0] == 'S') {
...
/*JP	    You("have violated the sanctity of this grave!");*/
    ...
    if (rn2(5) == 3) {
/*JP	pline("The grave is empty.  Strange....");*/
	pline("奇妙なことに墓の中は空だった．．．");
	/* explains all those zombies and mummies wandering around... */
	return;
    }
/*JPif(Blind) pline("You feel an icy draft...");
    else pline("You unearth a corpse.");*/
    if(Blind) pline("冷たい隙間風を感じた．．．");
    else pline("死体を掘り出した．");
    num_obj = rn2(3) + rn2(3) + rn2(3);
    for (o=0;o<num_obj;o++) {
       curse(mkobj_at(0,u.ux, u.uy, TRUE));
       mkgold(10*level_difficulty()+rn2(5),u.ux,u.uy);
    }
    switch (rn2(4)+1) {
/*JP	case 1: if (!Blind) pline(Hallucination ? "Dude!  The living dead!" :
		 "It is very upset!");*/
	case 1: if (!Blind) pline(Hallucination ? 
			"洒落てるなー！生きている死体だ！" :
			"うわー！死体が動いた！");
		 if (rn2(3)) (void) makemon(mkclass(S_ZOMBIE,0), u.ux, u.uy, NO_MM_FLAGS);
		 else (void) makemon(mkclass(S_MUMMY,0), u.ux, u.uy, NO_MM_FLAGS);
       break;
       default:
	    (void) mk_tt_object(CORPSE, u.ux, u.uy);
       break;
    }
    newsym(u.ux,u.uy);
}
...
/*JP		    pline("%s goes out!", Yname2(obj));*/
...
/*JP		    You("snuff out %s.", yname(obj));*/
		    You("%sを拭き消した．", xname(obj));
		}
		end_burn(obj, TRUE);
		return;
	}
...
/*JP			pline("This %s has no oil.", xname(obj));*/
...
/*JP	struct obj *obj = getobj(cuddly, "rub");*/
	...
	if (uwep->otyp == MAGIC_LAMP) {
	    if (uwep->spe > 0 && !rn2(3)) {
		/* [Tom] they have to charge... come on! */                
		if (obj->unpaid) {
/*JP		    verbalize("What are you trying to pull, pal? You owe me big time!");*/
		    verbalize("何を引き出そうとしているんだい君は？この代償は高くつくぞ！");
		    bill_dummy_object(obj);
		}
...
/*JP		You("cannot tin %s which is partly eaten.",something);*/
	...
	obj->spe--;        
	if (TURNSTONE(corpse->corpsenm)
	    && !resists_ston(&youmonst) && !uarmg) {
/*JPpline("Tinning %s without wearing gloves is a fatal mistake...",something);*/
pline("%sを小手なしで缶詰にするのは致命的な間違いだ．．．",something);
...
/*JP		    killer = "trying to tin something hazardous";*/
		    killer = "素手で石化能力のある怪物を缶詰にしようとして";
		    done(STONING);
		}
	}
...
/* -----------============STEPHEN WHITE'S NEW CODE============----------- */
/*JP   } else  pline("You seem to be out of tins.");*/
   } else  pline("あなたは缶詰で出来ているようだ．");
}
...
/* STEPHEN WHITE'S NEW CODE */
	if (trouble_count == 0 && !rn2(20) || (obj && obj->blessed) && !rn2(15)) {
...
/* STEPHEN WHITE'S NEW CODE */
	case ORB_OF_ENCHANTMENT:
		...
		if (uwep->spe < 5) {
		if (obj->blessed) {
/*JP				if (!Blind) pline("Your %s glows silver.",*/
				if (!Blind) pline("あなたの%sは銀色に輝いた．" ,
							xname(uwep));
				uwep->spe += rnd(2);
		} else if (obj->cursed) {                               
/*JP				if (!Blind) pline("Your %s glows black.",*/
				if (!Blind) pline("あなたの%sは黒い光を放った．" ,
							xname(uwep));
				uwep->spe -= rnd(2);
		} else {
		    if (rn2(3)) {
/*JP			if (!Blind) pline("Your %s glows bright for a moment." ,*/
			if (!Blind) pline("あなたの%sは一瞬明るく輝いた．" ,
						xname(uwep));
			uwep->spe += 1;
		    } else {
/*JP			if (!Blind) pline("Your %s glows dark for a moment.",*/
			if (!Blind) pline("あなたの%sは一瞬黒ずんだ光を放った．" ,
						xname(uwep));
			uwep->spe -= 1;
		    }
		}
/*JP		} else pline("Nothing seems to happen.");*/
  		} else pline("何も起きなかったようだ．");
		
		if (uwep->spe > 5) uwep->spe = 5;
				
/*JP		} else pline("The orb glows for a moment, then fades.");*/
		} else pline("オーブは一瞬輝きその光は次第に消えていった．");
		obj->spe--; 
	    
/*JP	    } else pline("This orb is burnt out.");*/
	    } else pline("このオーブは焦げている．");
	    break;
	case ORB_OF_CHARGING:
		if(obj->spe > 0) {
			...
			obj->spe--;
/*JP			otmp = getobj(all_count, "charge");*/
...
/*JP		} else pline("This orb is burnt out.");*/
	...
	case ORB_OF_DESTRUCTION:
		useup(obj);
/*JP		pline("As you activate the orb, it explodes!");*/
		pline("オーブを使おうとすると、それは爆発した！");
		explode(u.ux, u.uy, 11, d(12,6), WAND_CLASS);
		check_unpaid(obj);
		break;
	case MAGIC_MARKER:
		res = dowrite(obj);
...
/*JP    pline("Opening the tin will be much easier if you wield the tin opener.");*/
	...
	case PAN_PIPE:
		...
		break;
	case MEDICAL_KIT:        
	      if (pl_character[0] == 'H') can_use = TRUE;
	      else if ((pl_character[0] == 'P' || pl_character[0] == 'M' ||
			pl_character[0] == 'U' || pl_character[0] == 'S') &&
			!rn2(2)) can_use = TRUE;
		...
		check_unpaid(obj);
/*JP		You("take a few pills from your bag and swallow them.");*/
		You("バッグからピルをいくつか取り出すとそれを飲み込んだ．");
		obj->spe--;
		if(can_use) {
			...
			else if (Hallucination) make_hallucinated(0L, TRUE, 0L);
...
/*JP				You("have taken some bad pills!");*/
				You("品質の悪いピルを沢山飲んでしまった！");
			} else if (rn2(3)) {
/*JP				You("seem to have made your condition worse!");
				losehp(rn1(10,10), "a drug overdose", KILLED_BY);*/
				You("体調を更に悪化させてしまったようだ！");
				losehp(rn1(10,10), "薬物中毒で", KILLED_BY);
/*JP			} else pline("Nothing seems to happen.");
	      } else You("seem to be out of medical supplies");*/
			} else pline("何も起きなかったようだ．");
	      } else You("全く医療の準備ができてないようだ．");
	      break;
