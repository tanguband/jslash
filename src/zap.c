...
/*JP			   pline("%s slows down!", Monnam(mtmp));*/
			   pline("%sは動作がのろくなった．", Monnam(mtmp));
			}
		}
		break;
	case WAN_SPEED_MONSTER:
		if (!resist(mtmp, otmp->oclass, 0, NOTELL)) {
		   ...
		   if (canseemon(mtmp)) {                
/*JP			   pline("%s speeds up!", Monnam(mtmp));*/
			   pline("%sは動作が早くなった．", Monnam(mtmp));
		   }
		}
		break;
...
/*JP		pline("%s begins to look better.", Monnam(mtmp));*/
...
/*JP		     pline("%s suddenly panics!",Monnam(mtmp));*/
		     pline("突然%sはパニックに陥った！",Monnam(mtmp));
		     mtmp->mflee = 1;
		}
		break;
	case WAN_PROBING:
...
/*JP		    pline("%s looks%s better.", Monnam(mtmp),
		...
		/* Healing pets is a good thing ... except of Lycanthropes */                
...
/*JP		   You("feel much better.");*/
		...
		break;
		case WAN_FEAR:
/*JP		   You("suddenly panic!");*/
		   You("突然わけがわからなくなった！");
		   if(!HConfusion) make_confused(HConfusion + rnd(10),FALSE);
		    break;
		case WAN_SLEEP:
		    makeknown(WAN_SLEEP);
		case SPE_SLEEP:
...
/*JP	  pline("A mysterious force blocks your %s!",
		(obj->otyp == WAN_POLYMORPH) ? "wand" : "spell");*/
	  pline("奇妙な力があなたの%sを防いだ！",
		(obj->otyp == WAN_POLYMORPH) ? "杖の効力" : "呪文");
	  return;
	}
#endif /* BLACKMARKET */
...
register int dx,dy;
{
    ...
    if (type == ZT_LIGHTNING+1) type = ZT_SPELL(ZT_FIRE);
...
/*JP			if (Reflecting & W_AMUL) pline("But it reflects from your amulet!");
			else if (Reflecting & W_ARM) pline("But it reflects from your armor!");
			else pline("But it reflects from your shield!");*/
			/*pline("But it reflects from your %s!",
			      (Reflecting & W_AMUL) ? "amulet" : "shield");*/
			pline("しかし，それはあなたの%sによって反射された．",
			      (Reflecting & W_AMUL) ? "魔除け" :
			      (Reflecting & W_ARM) ? "鎧" : "盾");
		    } else
