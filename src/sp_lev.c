...
static void FDECL(create_object, (object *, struct mkroom *, int));
...
struct mkroom	*croom;
{
	...
	if (!class)
	    ...
	    if (flags.female && Role_is('E') && !u.uelf_drow && m->id == PM_EARENDIL)
		m->id = PM_ELWING;
 ...
 */
static void
create_object(o,croom,egg_type)
object	*o;
struct mkroom	*croom;
int egg_type;
{
...
/*              for(i = 0; i<r->ntoilet; i++)
		...
		for(i = 0; i<r->nobject; i++) {
		    create_object(r->objects[i], aroom, 0);
		}
		/* The gold piles */
		for(i = 0; i<r->ngold; i++)
...
dlb *fd;
{
	...
	while(n--) {
		...
		create_object(&tmpobj, (struct mkroom *)0, 0);
	}
