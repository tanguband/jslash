...
boolean telekinesis;	/* not picking it up directly by hand */
{
	...
	if (obj->oinvis && !See_invisible) obj->dknown = 1;
#endif
...
*/
	    ...
	    if ( TURNSTONE(obj->corpsenm) && !uarmg
			&& !resists_ston(&youmonst) && !telekinesis) {
		if (poly_when_stoned(uasmon) && polymon(PM_STONE_GOLEM))
		    display_nhwindow(WIN_MESSAGE, FALSE);
		else {
		  char tmpstr[64];
/*JP		  pline("Touching the corpse is a fatal mistake.");*/
		  pline("その死体に触れるのは致命的な間違いだ．");
		    Sprintf(tmpstr, "%sの死体に触れて", jtrns_mon(mons[obj->corpsenm].mname, -1));
		    instapetrify(tmpstr);
		    return -1;
		}
...
/*JP		Norep("You cannot %s %s you are wearing.",
			Icebox ? "refrigerate" : "stash", something);*/
		Norep("身につけているものを%sことはできない．", 
			Icebox ? "冷凍する" : "しまう");
		return 0;
	} else if ((obj->otyp == LOADSTONE || obj->otyp == STONE_OF_ROTTING) 
		&& obj->cursed) {
		obj->bknown = 1;
/*JP	      pline_The("stone%s won't leave your person.", plur(obj->quan));*/
...
register struct obj *obj;
{
	...
	if (obj->otyp == PICK_AXE && !obj->unpaid && *u.ushops && (shkp = shop_keeper(*u.ushops))) {
/*JP	  verbalize("You sneaky cad! Get out of here with that pick-axe!");*/
	  verbalize("つるはしを使って何をこそこそしているんだ！");
	}
...
/*JP	    You("owe %ld zorkmids for lost item%s.",
	...
	if (!cnt) {
/*JP	    pline("%s is empty.", Yname2(obj));*/
	    pline("%sは空っぽだ．", Yname2(obj));
	} else {
/*JP	    Sprintf(qbuf, "Do you want to take %s out of %s?",
		    something, yname(obj));*/
	    Sprintf(qbuf, "%sから%s取り出しますか？", yname(obj), something);
	    if (flags.menu_style != MENU_TRADITIONAL) {
