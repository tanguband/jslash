...
int flag;
{
    ...
    if (flag == 1)
	mon->mintrinsics |= (ptr->mresists & 0xFFFF);
    else
	mon->mintrinsics = (ptr->mresists & 0xFFFF);
    return;
}
...
player_mon()
{
	switch (u.role) {
		...
		case 'D': return &mons[PM_DOPPELGANGER];
		case 'E': {
		    if (u.uelf_drow) return &mons[PM_DROW];
		    return &mons[PM_ELF];
		}
		case 'F': return &mons[PM_FLAME_MAGE];
		case 'G': return &mons[PM_GNOME];
		case 'H': return &mons[PM_HEALER];
		case 'I': return &mons[PM_ICE_MAGE];
#ifdef JFIGHTER
		case 'J': return &mons[PM_JFIGHTER];
#endif
		case 'K': return &mons[PM_KNIGHT];
		case 'L': return &mons[PM_HUMAN_WEREWOLF];
		case 'M': return &mons[PM_MONK];
		case 'N': return &mons[PM_NECROMANCER];
...
#endif
		case 'U': return &mons[PM_UNDEAD_SLAYER];
