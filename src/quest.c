...
#endif
    /* convert Ruggo the Gnome king into a Leader */
    if (Role_is('G')) {
        mons[PM_RUGGO_THE_GNOME_KING].msound = MS_LEADER;
        mons[PM_RUGGO_THE_GNOME_KING].mflags2 |= (M2_PEACEFUL);
        mons[PM_RUGGO_THE_GNOME_KING].mflags3 = M3_CLOSE;
    }
    /* evil elf... */
    if (Role_is('E') && u.uelf_drow) {
    mons[PM_EARENDIL].msound = MS_NEMESIS;
    mons[PM_EARENDIL].mflags2 &= ~(M2_PEACEFUL);
    mons[PM_EARENDIL].mflags2 |= (M2_NASTY|M2_STALK|M2_HOSTILE);
    mons[PM_EARENDIL].mflags3 = M3_WANTSARTI | M3_WAITFORU;
    }
    /*
#endif
    if (Role_is('P')) {
