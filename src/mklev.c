...
static void FDECL(mktoilet,(struct mkroom *));
#endif
...
makelevel()
{
...

	{
...
#endif
	    {
		    ...
		    quest_ch = pl_character[0];
		    if (Role_is('E') && u.uelf_drow) quest_ch = 'Q';
		    Sprintf(fillname, "%c-locate", quest_ch);

		    loc_lev = find_level(fillname);

		    Sprintf(fillname, "%c-fill", quest_ch);
		    Strcat(fillname,
	...
	/* [Tom] totally reorganized this into categories... used
	   ...
	   else if(depth(&u.uz) > 12 && !rn2(12)) mkroom(LEPRECOURT);
	   ...
	   if(depth(&u.uz) > 9 && !rn2(12) &&
		(mons[PM_KILLER_BEE].geno & (G_GENOD | G_EXTINCT))) mkroom(BEEHIVE);
	     else if(depth(&u.uz) > 12 && !rn2(15)) mkroom(ANTHOLE);
	   /* zoos */
	   ...
	   /* dangerous ones */
	   if(depth(&u.uz) > 13 && !rn2(19)) mkroom(COCKNEST);
