...
register struct monst *mtmp;
{
	...
	switch(mndx) {
	    ...
	    case PM_VECNA:
		if (rn2(2)) {
/*JP		  pline("All that remains is a single eye...");*/
		  pline("片目が残っている．．．");
		  obj = mksobj_at(EYEBALL, x, y, TRUE);
		  obj = oname(obj, artiname(ART_EYE_OF_VECNA));
		  obj->quan = 1;
		  curse(obj);
		}
		else {
/*JP		  pline("All that remains is a hand...");*/
		  pline("手が残っている．．．");
		  obj = mksobj_at(SEVERED_HAND, x, y, TRUE);
		  obj = oname(obj, artiname(ART_HAND_OF_VECNA));
		  obj->quan = 1;
		  curse(obj);
		}
		fobj = obj;
...
#define mstoning(obj)	(ofood(obj) && (TURNSTONE((obj)->corpsenm) || \
...
/*JP			You("hear an awful gobbling noise!");*/
			You("がつがつと食べる音を聞いた．");
		    mtmp->meating = 2;
		    delobj(otmp);
...
struct monst *mon;
{
	...
	if (mdat == &mons[PM_VLAD_THE_IMPALER] ||
	    (mdat->mlet == S_LICH && !(mdat == &mons[PM_VECNA]))) {
...
/*JP		if (canseemon(mtmp)) pline("%s explodes!",Monnam(mtmp));*/
		if (canseemon(mtmp)) pline("%sは爆発した！",Monnam(mtmp));
		sprintf(buf, "%sの爆発", Monnam(mtmp));
		killer = buf;
		killer_format = KILLED_BY_AN;
		explode(x, y, AD_PHYS, tmp, MON_EXPLODE);
	   }
	}
...
/*JP	    if (!Hallucination) pline("That was probably a bad idea...");*/
	    if (!Hallucination) pline("たぶん悪い考えだ．．．");
/*JP	    else pline("Whoopsie-daisy!");*/
	    else pline("こりゃまいった！");
	}
...
/*JP		if (!Hallucination) pline("You hear the rumble of distant thunder...");*/
		if (!Hallucination) You("遠くで雷鳴を聞いた．．．");
/*JP		else pline("You hear the studio audience applaud!");*/
		else pline("あなたはスタジオ聴取者の拍手を聞いた！");
	}
	else if (mtmp->mpeaceful) {
		adjalign(-5);
/*JP		if (!Hallucination) pline("The gods will probably not appreciate this...");*/
		if (!Hallucination) pline("神はこの行ないを許さないないだろう．．．");
/*JP		else pline("Whoopsie-daisy!");*/
		else pline("こりゃまいった！");
	}
