...
#define NAMSZ	10
#define DTHSZ	60
#define PERSMAX	 4		/* entries per name/uid per char. allowed */
#define POINTSMIN	1	/* must be > 0 */
#define ENTRYMAX        250     /* must be >= 10 */
...
/*
  Y2K problem fixed
*/
	char deathdate[16], birthdate[16];
	int uid;
	char plchar;
	char sex;
	char name[NAMSZ+1];
...
NEARDATA const char *killed_by_prefix[] = {
	...
	"焼死した", "溶岩に溶けた", "押し潰された", "石化した", "死んだ",  "スライムになった", "虐殺された",
	"", "",
	"", "", ""
};
...
*/
#ifdef NO_SCAN_BRACK
	static char *fmt = "%d %d %d %ld %d %d %d %d %d %d %s %s %d%*c%c%c %s %s%*c";
#else
	static char *fmt = "%d.%d.%d %ld %d %d %d %d %d %d %s %s %d %c%c %[^,],%[^\n]%*c";
#endif
...
*/
			fmt = "%d.%d.%d %ld %d %d %d %d %d %d %s %s %d %c%c %[^,],%[^\n]%*c";
	}
#endif /* __BEOS__ */ 
...
/*
  Y2K problem fixed
*/
		if(atoi(tt->deathdate) < 1900)  /* OLD format */
		  Sprintf(tt->deathdate, "%08d", atoi(tt->deathdate) + 1900);
		if(atoi(tt->birthdate) < 1900)  /* OLD format */
		  Sprintf(tt->birthdate, "%08d", atoi(tt->birthdate) + 1900);
	}
}
...
#define HUP
	...
	switch (killer_format) {
		...
		case KILLED_BY_AN:
/*JP			if (Instant_Death) {
			  Strcat(t0->death, "instantly ");
			  strcat(t0->death, killed_by_prefix[how]);
			}
			else if (Quick_Death) {
			  Strcat(t0->death, "quickly ");
			Strcat(t0->death, killed_by_prefix[how]);
			}
			else Strcat(t0->death, killed_by_prefix[how]);*/
...
/*JP		Strcat(linebuf, "quit");*/
	    ...
	    } else if (!strncmp(t1->death, "turned to stone", 15)) {
		Strcat(linebuf, "turned to stone");
		second_line = FALSE;            
	    } else if(!strncmp(t1->death, "turned into slime",17)) {
		Strcat(linebuf, "turned into slime");
		second_line = FALSE;
...
boolean fem;
{
	switch (plch) {
		...
		case 'D': return PM_DOPPELGANGER;                
		case 'E': {
		   if (u.uelf_drow) return PM_DROW;
		   return PM_ELF;
		}
		case 'F': return PM_FLAME_MAGE;
		case 'G': return PM_GNOME;
		case 'H': return PM_HEALER;
		case 'I': return PM_ICE_MAGE;
#ifndef JFIGHTER
  		case 'J':	/* accept old Fighter class */
#else
		case 'J': return PM_JFIGHTER;
#endif
		case 'K': return PM_KNIGHT;
		case 'L': return PM_HUMAN_WEREWOLF;                
		case 'M': return PM_MONK;
		case 'N': return PM_NECROMANCER;
...
#endif
		case 'U': return PM_UNDEAD_SLAYER;                
