...
#define LUCKMIN		(-10)
	...
	int ugangr;			/* if the gods are angry at you */
        int usacrifice;                 /* number of sacrifeces so far */
	...
	boolean twoweap;                /* two-weapon combat */
#endif /* WEAPON_SKILLS */

};	/* end of `struct you' */

#define Role_is(X) (u.role == X)
#define human_role() (!Role_is('E') && !Role_is('G'))
#define Upolyd (u.mtimedone != 0)

/* these roles qualify for a martial arts bonus */
#define martial_bonus()	(Role_is('S') || Role_is('P') || Role_is('M'))
